...
export function ArtworkImagesDBProvider(props: ParentProps) {
	const [artworks] = createResource<ArtworkImagesDB>(async function f() {
		...
		const baseURL = import.meta.env.PROD ? `https://storage.googleapis.com/${db.bucketName}` : ''

		db.getImageURL = (id: string, size: string) => `${baseURL}/artworks/images/${size}/${id}.webp`

		return db
	})
	return <ArtworkImagessDBContext.Provider value={artworks}>{props.children}</ArtworkImagessDBContext.Provider>
}
