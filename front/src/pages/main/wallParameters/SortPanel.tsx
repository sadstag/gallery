...
export const SortPanel = () => {
	const {
		wallModel,
		operations: { invertSortDirection, setSort },
	} = useWallModel()

	return (
		<div class={styles.sortPanel}>
			<Select
				value={wallModel.sort.on}
				onChange={value => {
					setSort({
						on: value as SortType,
						direction: preferedDirectionForSortType[value as SortType],
					})
				}}
				options={wallModel.availableSorts.map(sortType => ({ value: sortType, label: sortTypeLabels[sortType] }))}
			/>
			<Button onClick={invertSortDirection}>
				<SortIcon
					classList={{ [styles['sort-icon']]: true, [styles.descending]: wallModel.sort.direction === 'desc' }}
				/>
			</Button>
		</div>
	)
