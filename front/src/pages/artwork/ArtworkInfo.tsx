...
const fieldValueTransformers: {
	...
	available: (v: ValueType) => <span class={styles[v ? 'available' : 'not-available']}>{i18n(v ? 'yes' : 'no')}</span>,
}

const getTransformer = (field: keyof Artwork) => fieldValueTransformers[field] ?? defaultTransformer

export function ArtworkInfo(props: Props) {
	const [{ artwork }] = splitProps(props, ['artwork'])

	const infos = createMemo(() =>
		artworkInfoFields.reduce((accInfos: Info[], field: keyof Artwork) => {
	...
	return (
		<div class={styles['info-blocs-container']}>
			<For each={infos()}>{([label, value]) => <InfoBloc label={label} value={value} />}</For>
		</div>
	)
}
